...
Currently configured project: `firmowid.security-audit` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/firmowid.security-audit/SETUP.md` to fill in
   `data/firmowid.security-audit/INFO.md`.
...
`--project-id firmowid.security-audit` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/firmowid.security-audit/`.

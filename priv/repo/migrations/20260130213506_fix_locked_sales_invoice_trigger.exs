defmodule Firmowid.Repo.Migrations.FixLockedSalesInvoiceTrigger do
  @moduledoc """
  Updates the locked_sales_invoice_trigger to reflect schema changes:
  - Removed: is_basic_info_confirmed, is_seller_confirmed, is_buyer_confirmed, are_sales_invoice_items_confirmed
  - Renamed: buyer_name -> buyer_given_name
  - Added: buyer_full_name, counterparty_id
  """
  use Ecto.Migration

  def up do
    # Update the trigger function to use current column names
    execute """
    ...
    BEGIN
      ...
      IF TG_OP = 'UPDATE' AND OLD.locked_at IS NOT NULL THEN
        IF ROW(
          ...
          OLD.is_cash_account, OLD.is_reverse_charge, OLD.skip_invoicing, OLD.item_names,
          ...
          NEW.is_cash_account, NEW.is_reverse_charge, NEW.skip_invoicing, NEW.item_names,
  ...
  def down do
    # Restore the old trigger function (with old column names)
    execute """
    CREATE OR REPLACE FUNCTION prevent_locked_sales_invoice_modification()
    RETURNS TRIGGER AS $$
    BEGIN

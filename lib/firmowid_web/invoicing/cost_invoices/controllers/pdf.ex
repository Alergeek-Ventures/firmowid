defmodule FirmowidWeb.Invoicing.CostInvoices.Controllers.Pdf do
  ...
  def pdf(conn, %{"id" => id}) do
    ...
    case CostInvoice.by_id(id,
           scope: conn.assigns.ash_scope,
           load: [:effective_seller_display_name, blob: []]
         ) do
      {:ok, invoice} ->
        case CostInvoicePdf.generate(invoice,
          ...
          {:ok, pdf_binary} ->
            filename = MonthDownloadEntries.cost_invoice_pdf_filename(invoice)
          ...
          {:error, reason} ->
            Logger.error("Cost invoice PDF generation failed in controller for #{id}: #{inspect(reason)}")

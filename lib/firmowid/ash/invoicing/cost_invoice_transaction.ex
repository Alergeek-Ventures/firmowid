defmodule Firmowid.Ash.Invoicing.CostInvoiceTransaction do
  ...
  Replaces the `create_cost_invoices_transactions_connection/3` and
  `delete_cost_invoices_transactions_connections/1` functions from the
  legacy `CostInvoices` context.
  """
  use Ash.Resource,
  ...
  actions do
      ...
      run fn input, _context ->
        ...
        result =
          ...
          |> Enum.reduce(Ecto.Multi.new(), fn %{changes: data} = changeset, acc ->
            Ecto.Multi.insert(acc, {data.cost_invoice_id, data.transaction_id}, changeset)
          end)
    ...
    action :delete_for_invoice, :term do
      ...
      run fn input, _context ->
        ...
        query =
          from(c in CostInvoicesTransactions,
            where: c.cost_invoice_id == ^input.arguments.invoice_id
          )

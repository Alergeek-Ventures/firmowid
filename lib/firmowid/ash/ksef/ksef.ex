defmodule Firmowid.Ash.Ksef do
  ...
  """
  @spec authenticate_with_uploaded_certificate(
          String.t(),
          String.t(),
          String.t() | nil,
          Scope.t()
        ) :: {:ok, Credential.t()} | {:error, term()}
  def authenticate_with_uploaded_certificate(certificate, private_key, private_key_password, scope) do
    authenticate_with_ksef_certificate(
      certificate,
      private_key,
  ...
  @doc """
  Authenticates with a KSeF certificate generated by Firmowid.

  Its origin is retained so the certificate can be revoked when the
  organization disconnects from KSeF.
  """
  @spec authenticate_with_generated_certificate(
          String.t(),
          String.t(),
          Scope.t()
        ) :: {:ok, Credential.t()} | {:error, term()}
  def authenticate_with_generated_certificate(certificate, private_key, scope) do
    authenticate_with_ksef_certificate(
      certificate,
      private_key,
      nil,
  ...
  defp authenticate_with_ksef_certificate(certificate, private_key, private_key_password, auth_type, scope) do
    org_id = scope.tenant
    opts = [scope: scope]
    organization = Core.get_organization!(org_id, opts)
  ...
  def unauthenticate(scope) do
    ...
    case get_credential(scope) do
      ...
      credential ->
        access_token = SessionWorker.get_access_token!(org_id)

        with :ok <- maybe_revoke_generated_certificate(credential, access_token),
             :ok <- ApiClient.revoke_refresh_token(access_token),
             {:ok, _} <- SessionWorker.invalidate_access_token(org_id) do
          Repo.transact(fn ->
            ...
            Credential.destroy!(credential, scope: scope)
            {:ok, :disconnected}
          end)
        end
    end
  ...
  defp maybe_revoke_generated_certificate(%Credential{}, _access_token), do: :ok
